...
custom quality rules**, **99.80% pass, 24 rows quarantined** to
...
- **[SAY · Alan]** "Here's the part that matters: we didn't translate this contract
  into some other rule language. **DQX reads the ODCS contract natively** — as of
  v0.11 last December, the contract *is* the rule source. So there's no second
  artifact to drift out of sync. The thing the producer agreed to is the exact thing
  that runs."
- *(Optional credibility beat, cite the date — signals you're current.)*
