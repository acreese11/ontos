...
async def require_ontos_admin(
    ...
    Ported from upstream databrickslabs/ontos#404/#458 - use this for routes
    that grant capabilities beyond a single feature, e.g. MCP token management
    (mcp_tokens_routes.py), which previously (and incorrectly) gated on
    `settings:READ_WRITE`, letting any user with Settings write access mint
    their own MCP tokens with arbitrary scopes.
    """
