...
class AuthorizationManager:
    ...
    def is_user_ontos_admin(self, user_groups: Optional[List[str]]) -> bool:
        ...
        canonical "Ontos admin" check for gating admin-only capabilities such as
        MCP token management - intentionally decoupled from ``settings:ADMIN``
        (a user may administer Settings without being an Ontos admin, and vice
        ...
        Returns:
            ...
            empty-groups case, and on any lookup failure - fails closed).
        """
        if not user_groups:
            return False
